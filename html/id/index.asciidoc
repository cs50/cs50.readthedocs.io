...
Once you've installed Python, Django, and pip, download a copy of the
CS50 library using pip via:
...
[source,bash]
-------------------------------------
$ django-admin.py startproject cs50id
-------------------------------------

Next, create an app within your project, like `sample`, from within the
...
The first thing we must do with our app is configure our settings so we
can index dictionaries within our templates. Insert the following into the
settings.py file located within your project's similarly named subfolder:
...
Next, you must set up your project to use a database of some kind, be it
SQLite, MySQL, PostgreSQL, Oracle, or other extended databases supported by
Django. The choice and pathname of this database can be found in settings.py,
within the DATABASES tuple. Finalize this process with the following command
...
sample) and fill it with the following information:
...
urlpatterns = patterns('',
    ...
    url(r'^return_page/$', views.return_page, name='return_page'),
)
-----
...
our document root that is named after the project itself (i.e., `cs50id/cs50id` in
this case):
...
following, assuming our project is titled cs50id:

`cs50id/sample/templates/sample/index.html`

(As an aside and can be read by browsing the Django documentation,
packaging our templates within a subfolder of our project named the
same as our project results in a more modular design and the ability
to repackage apps for future use, so we have written this example
...
{% if request.session.user %}
    ...
    You are logged in. <a href="/sample/logout/">Log out.</a><br />
...
{% else %}
    ...
    You are not logged in. <a href="/sample/login/">Log in.</a>
...
def login(request):
    if 'user' in request.session:
        ...
        return HttpResponseRedirect(CS50.getLoginUrl('tmp', 'http://127.0.0.1:8000', 
            'http://127.0.0.1:8000/sample/return_page/', request.session))
...
-----
...
in this case a view we have titled return_page and will see soon. Also note the 
...
In this case, "return_page" is chosen as the name of this view in favor of "return"
...
def return_page(request):
    user = CS50.getUser('tmp', 'http://127.0.0.1:8000/sample/return_page/', request, 'sample:return_page')
...
an updated message about us being logged in with an ID, as well as our credentials.
