...
17. Finally, you are ready to package the machine! Make sure it is properly shut down before the following steps.

*VMware*: Find the virtual machine in the Finder by right-clicking the virtual machine in the Virtual Machine Library and selecting *Show in Finder*. 
*VirtualBox*: Export the virtual machine by selecting it in the Oracle VM VirtualBox Manager then selecting *File > Export Appliance..*. Click *Continue*. Choose a location for the export, and be sure to select *Open Virtualization Format (.ovf)* when selecting a file name and location. Click *Continue* and then *Export*. Find the exported virtual machine.

*Both*: Take *only* the `appliance50.vmdk` file from each, and package it with a fresh version of the VMX or OVF file (as appropriate). This fresh version should be newly downloaded, as in step 15. You should move each VMDK file to a fresh directory that houses just the VMDK and the VMX or OVF, as appropriate. Finally, zip up the machine (`zip -r` in Terminal), and it is ready to go!
