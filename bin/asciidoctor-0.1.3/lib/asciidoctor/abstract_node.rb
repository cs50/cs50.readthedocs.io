...
class AbstractNode
  ...
  def generate_data_uri(target_image, asset_dir_key = nil)
    ...
    if !File.readable? image_path
      $stderr.puts "asciidoctor: WARNING: image to embed not found or not readable: #{image_path}"
  ...
  def read_asset(path, warn_on_failure = false)
    if File.readable? path
      ...
      $stderr.puts "asciidoctor: WARNING: file does not exist or cannot be read: #{path}" if warn_on_failure
      nil
    end
  end
