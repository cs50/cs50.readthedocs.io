...
`http://api.cs50.net/courses/1.0/courses?key=KEY&cat_num=4949&output=jsonp&callback=parseResponse`, where `KEY` is your API key),
...
PHP format (e.g.,
`http://api.cs50.net/courses/2/courses?cat_num=4949&output=php`, where `KEY` is your API key), the
...
* http://wiki.cs50.net.php?title=CS50 Courses_API&oldid=3204[0.9]
* 1.0
