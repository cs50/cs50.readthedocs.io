...
module Substituters
  ...
  def apply_subs(lines, subs = COMPOSITE_SUBS[:normal])
    ...
    subs.each {|type|
      ...
      when :post_replacements
        ...
        $stderr.puts "asciidoctor: WARNING: unknown substitution type #{type}"
      end
    }
      ...
      line = line.gsub(REGEXP[:attr_ref]) {
        ...
        if !m[1].nil? || !m[4].nil?
          ...
          when 'counter', 'counter2'
            ...
            $stderr.puts "asciidoctor: WARNING: illegal attribute directive: #{m[2]}"
            ''
          end
  ...
  def resolve_subs(subs)
    ...
    if (invalid = candidates - resolved).size > 0
      $stderr.puts "asciidoctor: WARNING: invalid passthrough macro substitution operation#{invalid.size > 1 ? 's' : ''}: #{invalid * ', '}"
    end 
    resolved
  end
