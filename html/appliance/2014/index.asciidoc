...
* If you have a *PC running a 64-bit edition of Windows or Linux*, we recommend that you follow link:workstation/[these instructions for VMware Workstation 11]. footnote:[If your Windows or Linux computer runs a 32-bit OS, please drop a note to sysadmins@cs50.harvard.edu.] footnote:[If you are running your own 64-bit Ubuntu Linux, you can run `wget -q -O - http://mirror.cs50.net/appliance50/2014/debs/keyFile | sudo apt-key add -` followed by `sudo add-apt-repository "deb http://mirror.cs50.net/appliance50/2014/debs/dists/trusty/main/binary-i386 /"`, after which `sudo apt-get install check50 library50-c style50` will install check50, the CS50 Library, and style50, respectively.]
