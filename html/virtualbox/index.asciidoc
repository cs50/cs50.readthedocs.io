...
link:../appliance[CS50 Appliance]) run unbearably slow within
...
link:../appliance[CS50 Appliance] via HTTP, SSH, and the like.) To
...
error (e.g., the link:../appliance[CS50 Appliance]), then click
*Settings*.
7.  Click *Network*.
...
link:../appliance[CS50 Appliance] for *Adapter 2*), ensure that the
...
(e.g., the link:../appliance[CS50 Appliance]); it should now be gone.

[[windows_nonexistent_host]]
...
If you find that virtual machines (like the link:../appliance[CS50
Appliance]) run unbearably slow within VirtualBox, you might need to
enable link:../Hardware_Virtualization[hardware virtualization] on your
computer.

[[]]
=== Troubleshooting

[[linux_nonexistent_host]]
==== Nonexistent host networking interface, name '' (VERR_INTERNAL_ERROR)
