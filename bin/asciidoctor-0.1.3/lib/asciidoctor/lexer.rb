...
class Lexer
      ...
      if next_level
        ...
        if next_level > current_level || (section.is_a?(Document) && next_level == 0)
          if next_level == 0 && doctype != 'book'
            ...
            $stderr.puts "asciidoctor: WARNING: line #{reader.lineno + 1}: section title out of sequence: " +
      ...
      if delimited_blk_match = is_delimited_block?(this_line, true)
        ...
        if !style
          ...
          if delimited_blk_match.masq.include? style
            ...
            $stderr.puts "asciidoctor: WARNING: line #{reader.lineno}: invalid style for #{block_context} block: #{style}"
        ...
        while true
          ...
          if (match = this_line.match(REGEXP[:colist]))
            ...
            begin
              ...
              if match[1].to_i != expected_index
                $stderr.puts "asciidoctor: WARNING: line #{reader.lineno + 1}: callout list item index: expected #{expected_index} got #{match[1]}"
              end
              ...
              if !list_item.nil?
                ...
                if !coids.empty?
                  ...
                  $stderr.puts "asciidoctor: WARNING: line #{reader.lineno}: no callouts refer to list item #{items.size}"
                end
              end
          ...
          # FIXME create another set for "passthrough" styles
            ...
            if PARAGRAPH_STYLES.include?(style)
              ...
              $stderr.puts "asciidoctor: WARNING: line #{reader.lineno}: invalid style for paragraph: #{style}"
              style = nil
  ...
  def self.resolve_ordered_list_marker(marker, ordinal = 0, validate = false)
    ...
    if validate && expected != actual
      ...
      $stderr.puts "asciidoctor: WARNING: list item index: expected #{expected}, got #{actual}"
    end
