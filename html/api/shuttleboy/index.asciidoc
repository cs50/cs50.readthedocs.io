...
David J. Malan '99 +
mailto:dmalan@harvard[dmalan@harvard.edu] +
...
| callback | _Required iff *output* is *jsonp*_. Callback function with which response will be padded.
| output | _Required._ Format for output. Must be `csv`, `json`, `jsonp`, or `php`.
|===
